...
module.exports.wardrobe = function (sex, where, equip){
  ...
  if (equip.indexOf('płaszcz') !== -1) { if(sex === "kobieta"){ where.text( "Stara, drewniana szafa, śmierdząca kurzem, pleśnią i niewiadomo czym jeszcze. Otworzyłaś szafę. Jest pusta.");  closeWardrobe(where);
  }else if(sex === "mężczyzna" || sex === "nie wiadomo"){ where.text("Stara, drewniana szafa, śmierdząca kurzem, pleśnią i niewiadomo czym jeszcze. Otworzyłeś szafę. Jest pusta.");   closeWardrobe(where); } }

//gdy nie płaszcza w ekwipunku
  else{ if(sex === "kobieta"){where.text( "Stara, drewniana szafa, śmierdząca kurzem, pleśnią i niewiadomo czym jeszcze. Otworzyłaś szafę, w której wisi płaszcz."); btnTakeCoat(where);
  closeWardrobe(where);
 }else if(sex === "mężczyzna" || sex === "nie wiadomo"){ where.text("Stara, drewniana szafa, śmierdząca kurzem, pleśnią i niewiadomo czym jeszcze. Otworzyłeś szafę, w której wisi płaszcz.");
...
//funkcje dla skrzyni
//zamykanie skrzyni
module.exports.closeChest = function(where){ $("#closeChest").on("click", function(){    $("#description").empty(); }); }

//funkcja tworzenia przycisku zamykania skrzyni
 function closeChest(where){ let closeChest = document.createElement("button"); closeChest.id = "closeChest"; closeChest.innerText = "zamknij skrzynię"; where.append(closeChest); }

//funkcja tworzenia przycisku zabierania złota
function btnTakeGold(where){ let takeGold = document.createElement("button"); takeGold.id = "takeGoldChest"; takeGold.innerText = "weź złoto"; where.append(takeGold); }
